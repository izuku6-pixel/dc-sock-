...
class MyClient(discord.Client):
    ...
    def configure(self):
        ...
        default_hint = f" (default: {MESSAGES_PATH})" if os.path.exists(MESSAGES_PATH) else ""
        if self.use_different_messages:
            for server in self.servers:
                message_path = log.question(f"Enter the stock message or path for Guild {server['guild_id']} Channel {server['channel_id']}{default_hint}: ").strip()
                if not message_path and os.path.exists(MESSAGES_PATH):
                    message_path = MESSAGES_PATH
                self.stock_messages[(server['guild_id'], server['channel_id'])] = message_path
        else:
            user_input = log.question(f"Enter the stock message or path{default_hint}: ").strip()
            if not user_input and os.path.exists(MESSAGES_PATH):
                self.stock_path = MESSAGES_PATH
            else:
                self.stock_path = user_input
